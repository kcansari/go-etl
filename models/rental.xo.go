...
func (r *Rental) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE public.rental SET ` +
		`rental_date = $1, inventory_id = $2, customer_id = $3, return_date = $4, staff_id = $5, last_update = $6 ` +
		`WHERE rental_id = $7`
	// run
...
func (r *Rental) Delete(ctx context.Context, db DB) error {
	...
	const sqlstr = `DELETE FROM public.rental ` +
		`WHERE rental_id = $1`
	// run
...
// RentalByInventoryID retrieves a row from 'public.rental' as a [Rental].
//
// Generated from index 'idx_fk_inventory_id'.
func RentalByInventoryID(ctx context.Context, db DB, inventoryID int) ([]*Rental, error) {
	// query
	const sqlstr = `SELECT ` +
		`rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update ` +
		`FROM public.rental ` +
...
func RentalByRentalDateInventoryIDCustomerID(ctx context.Context, db DB, rentalDate time.Time, inventoryID int, customerID int16) (*Rental, error) {
	// query
	const sqlstr = `SELECT ` +
		`rental_id, rental_date, inventory_id, customer_id, return_date, staff_id, last_update ` +
		`FROM public.rental ` +
...
func (r *Rental) Customer(ctx context.Context, db DB) (*Customer, error) {
	return CustomerByCustomerID(ctx, db, int(r.CustomerID))
}
...
func (r *Rental) Staff(ctx context.Context, db DB) (*Staff, error) {
	return StaffByStaffID(ctx, db, int(r.StaffID))
}

...
func (s *Store) Update(ctx context.Context, db DB) error {
	...
	const sqlstr = `UPDATE public.store SET ` +
		`manager_staff_id = $1, address_id = $2, last_update = $3 ` +
		`WHERE store_id = $4`
	// run
...
func StoreByManagerStaffID(ctx context.Context, db DB, managerStaffID int16) (*Store, error) {
	// query
	const sqlstr = `SELECT ` +
		`store_id, manager_staff_id, address_id, last_update ` +
		`FROM public.store ` +
...
func StoreByStoreID(ctx context.Context, db DB, storeID int) (*Store, error) {
	// query
	const sqlstr = `SELECT ` +
		`store_id, manager_staff_id, address_id, last_update ` +
		`FROM public.store ` +
...
func (s *Store) Staff(ctx context.Context, db DB) (*Staff, error) {
	return StaffByStaffID(ctx, db, int(s.ManagerStaffID))
}
